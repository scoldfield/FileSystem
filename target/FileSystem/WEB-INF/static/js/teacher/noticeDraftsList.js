...
require(['jquery', 'function', 'base', 'ymPrompt', 'mypannel', 'Pagination', 'emoji'], function(jquery){
    ...
    var options = {
        ...
        ajaxurl: window.globalPath + '/schoolnotice/draftJson',
        render: function(resObj){
            ...
            if(!resObj || typeof resObj.total == 'undefined' || resObj.total == 0){
                $trlist = '<tr><td colspan="5"><div class="f-cr">暂没有消息公告</div> </td></tr>'
            }else{
                ...
                for(var i = 0; i < listlen; i++){
                    ...
                    $trlist += ' <tr><td>' +
                        ' <label class="checkbox-ui"><input type="checkbox" value="' + listSingle.id + '"/><b></b></label>' +
                        ' </td>' +
                        ' <td><div class="simpleOverContent emoji">' + listSingle.content + '</div></td>' +
                        ' <td>共' + listSingle.total + '人</td>' +
                        ' <td>' + listSingle.time + '</td>' +
                        ' <td><a class="long" href="' + window.globalPath + '/' + _pathUrl + '/updatedeaft?id=' + listSingle.id + '">修改</a></td>' +
                        ' </tr>'
                }
            }
            ...
            ymPrompt.confirmInfo({
                message: html, width: 450, height: 300, handler: function(res){
                    if(res == 'ok'){
                        ...
                        $.ajax({
                            url: window.globalPath + '/schoolnotice/batchDelete',
                            data: {"ids": deleteStr},
                            type: 'POST',
                            dataType: 'json',
